#!/usr/bin/env sh

# Common
MOUNT_DATA_DIR="/mnt/disks/nodes/polygon"
MOONSTREAM_NODE_ETHEREUM_IPC_ADDR="${MOONSTREAM_NODE_ETHEREUM_IPC_ADDR:-http://127.0.0.1}"
MOONSTREAM_NODE_ETHEREUM_IPC_PORT="${MOONSTREAM_NODE_ETHEREUM_IPC_PORT:-18370}"

# Heimdall
SEEDS_LINE='seeds = "f4f605d60b8ffaaf15240564e58a81103510631c@159.203.9.164:26656,4fb1bc820088764a564d4f66bba1963d47d82329@44.232.55.71:26656"'

# Bor
BOR_BOOTNODES="enode://0cb82b395094ee4a2915e9714894627de9ed8498fb881cec6db7c65e8b9a5bd7f2f25cc84e71e89d0947e51c76e85d0847de848c7782b13c0255247a6758178c@44.232.55.71:30303,enode://88116f4295f5a31538ae409e4d44ad40d22e44ee9342869e7d68bdec55b0f83c1530355ce8b41fbec0928a7d75a5745d528450d30aec92066ab6ba1ee351d710@159.203.9.164:30303"
